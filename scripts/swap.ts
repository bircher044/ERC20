...
const USDCAmountToSwap = 7 * (10 ** 6); 

const main = async () => {

    const signer = new ethers.Wallet((String(process.env.BUYER_PRIVATE_KEY)), ethers.provider);

    const USDO = USDO__factory.connect(StablecoinContractAddress, signer);
    const USDC = USDC__factory.connect(StablecoinContractAddress, signer);
    //await console.log(await USDC.balanceOf(await signer.getAddress()));
    await console.log(await USDC.allowance(await signer.getAddress(), USDOContractAddress));
